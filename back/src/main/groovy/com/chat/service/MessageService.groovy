...
class MessageService {
    ...
    Mono<Void> updateMessagesForUser(String oldUsername, String newUsername) {
        return messageRepository.findByUsername(oldUsername)
                .flatMap(message -> {
                    message.setUsername(newUsername);
                    return messageRepository.save(message);
                })
                .then(); // Return Mono<Void> after the update is complete
    }
