...
class ChatWebSocketHandler implements WebSocketHandler {
    ...
    private final Sinks.Many<String> messageSink = Sinks.many().multicast().directAllOrNothing();
   ...
   ChatWebSocketHandler(UserRepository userRepository, MessageService messageService,UserService userService) {
    ...
    private Mono<Void> processSession(WebSocketSession session, SecurityContext securityContext) {
        ...
        if (authentication != null && authentication.isAuthenticated()) {
            ...
            Flux<String> messageFlux = messageSink.asFlux()
                    .doOnNext(msg -> logger.info("Broadcast message: " + msg))
                    .doOnSubscribe(subscription -> {
                        logger.info("New client subscribed to messageFlux");
                    })
                    .doOnTerminate(() -> {
                        logger.info("Client unsubscribed from messageFlux");
                    })
                    .doFinally(signalType -> {
                        logger.info("messageFlux completed with signal: " + signalType);
                    });
    ...
    private void broadcastMessage(String messageJson) {
        ...
        Sinks.EmitResult result = messageSink.tryEmitNext(messageJson);
